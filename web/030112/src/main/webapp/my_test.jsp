    ...
    <% 
	    ...
	    %>
	                <tr>
	                    ...
	                    <td>
	                        <button onclick="checkAnswer('<%= rs.getString("정답여부") %>')">정답[<%= rs.getString("정답여부") %>]</button>
	                    </td>
	                </tr>
	    <%
					...
					<%}%>
	               	<tr>
					    ...
					    <td>
					        ...
					        <button onclick="checkAnswer2(document.querySelector('input[name=answer_<%= rs.getString("seq") %>]:checked').value, '<%= rs.getString("정답여부").equals("Yes") ? "O" : "X" %>')">정답 확인<%= rs.getString("정답여부") %></button>
					    </td>
					</tr>
	    			<%	            	
	               ...
	               <tr bgcolor='skyblue'>
					...
					<tr>
					    <td colspan="2">
					        <!-- 사용자 답안 입력 필드 -->
					        <input type='text' id='answer_<%= rs.getString("seq") %>' value="" size='50'><br><%= rs.getString("보기") %>
					    </td>
					    <td>
					...
					%>
					<tr>
					    <td colspan="2"><%= bogi %><%= rs.getString("보기") %></td>
					    <td colspan="2">
					    <% 
			           ...
			           // out.println(sql);
						...
						<label>
				            ...
				            <%if (rs_link.getString("연결형답").equals(rs.getString("tb_test_sub_seq"))) { %>
				            [정답]
				            <%} %>
				        </label><br>
						<%
